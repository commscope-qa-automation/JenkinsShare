...
def executeRFMultiple(def script, dataList) {
    ...
    */  
    ...
    if (script.isUnix()) {
        sh 'docker exec ${DOCKER_CONTAINER} rebot --processemptysuite --outputdir "${RF_RESULT_DIR}" --report Final_report.html --name Final_output.xml ${RF_RESULT_DIR}/*_output.xml'
    } else {
        bat 'docker exec %DOCKER_CONTAINER% rebot --processemptysuite --outputdir "%RF_RESULT_DIR%" --report Final_report.html --name Final_output.xml %RF_RESULT_DIR%/*_output.xml'
    }  
}