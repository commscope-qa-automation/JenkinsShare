...
def executeRFMultiple(def script, dataList) {
    ...
    */  
    ...
    if (script.isUnix()) {
        sh 'docker exec ${DOCKER_CONTAINER} rebot --processemptysuite --outputdir "${RF_RESULT_DIR}" --output Final_output.xml --report Final_report.html --xunit Final_junit.xml "${RESULT_DIR}/*_output.xml"'
    } else {
        bat 'docker exec %DOCKER_CONTAINER% rebot --processemptysuite --outputdir "%RF_RESULT_DIR%" --output Final_output.xml --report Final_report.html --xunit Final_junit.xml "%RESULT_DIR%/*_output.xml"'
    }  
}